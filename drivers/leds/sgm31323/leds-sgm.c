...
//13种配置
static const struct time_source brightness_source[MAX_COLOR_DEFINE] = {
/*     r  h  f  o   r-ms  hms    fms     oms     */
	{  0, 0, 0, 0,  40,   40,    40,    40,   },
	{  6, 0, 6, 0,  1040, 40,    1040,  40,   },
	{  6, 2, 6, 2,  1040, 260,   1040,  260,  },
	{  7, 2, 7, 2,  1600, 260,   1600,  260,  },
	{  8, 0, 8, 0,  2100, 40,    2100,  40,   },
	{  2, 3, 0, 3,  260,  380,   40,    380,  },
	{  2, 2, 0, 3,  260,  260,   40,    380,  },
	{  1, 2, 0, 3,  130,  260,   40,    380,  },
	{  1, 2, 0, 2,  130,  260,   40,    260,  },
	{  0, 2, 0, 2,  40,   260,   40,    260,  },
	{  4, 0, 4, 0,  510,  40,    510,   40,   },
	{  0, 4, 0, 4,  40,   510,   40,    510,  },
	{  5, 2, 5, 2,  770,  260,   770,   260,  },
};
...
int sgm31323_set_blink_param(struct sgm_led *led)
{
	...
	for (i = 0; i < MAX_COLOR_DEFINE; i++) {
		if (led->pdata->rise_time_ms == brightness_source[i].index &&
			led->pdata->hold_time_ms == brightness_source[i].index1 &&
			led->pdata->fall_time_ms == brightness_source[i].index2 &&
			led->pdata->off_time_ms == brightness_source[i].index3) {

			rise_time_ms  = brightness_source[i].raise_time;
			hold_time_ms = brightness_source[i].hold_time;
			fall_time_ms = brightness_source[i].fall_time;
			off_time_ms = brightness_source[i].off_time;
			AW2027_DEBUG("found \n");
			break;
		}
	}
	...
	//reg1_value = reg1_value | 0x80; //reg01 的bit7 写1 ，换成线性模式。
	...
	SN_IIC_WriteReg(5, reg5_total_value);	//reg5  rase time、fall time
...
static int cale_brightness_linshi(struct sgm_led *led)
{
	int bright_tmp;
	...
	if (led->cdev.brightness > 0xff)
		led->cdev.brightness = 0xc0;
	if (led->cdev.brightness > 0xf)
		bright_tmp = led->cdev.brightness * 100 / 0xff * 0xc0 / 100;
	else
		bright_tmp = led->cdev.brightness * 100 / 0xf * 0xc0 / 100;
...
static void sgm_brightness(struct sgm_led *led)
{
	...
	if (led->pdata->led_mode == AW2027_LED_BREATH_MODE_MASK) {
		bright_tmp = cale_brightness_linshi(led);
	} else {
...
static struct led_trigger sgm_led_trigger[LED_MAX_NUM] = {
	{
		.name = "cc_mode",
