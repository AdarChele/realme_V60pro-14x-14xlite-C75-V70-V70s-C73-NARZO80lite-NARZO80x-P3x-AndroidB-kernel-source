...
enum IMGSENSOR_RETURN imgsensor_hw_init(struct IMGSENSOR_HW *phw)
{
    ...
    for (i = 0; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++) {
        PK_PR_ERR("IMGSENSOR_SENSOR_IDX: %d\n", i);
        ...
        char *pcurr_idx)
{
    ...
    while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
        ...
        if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON) {
            if (ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
                pr_info("[%s] sensor_idx:%d pwr_status:%d pin:%d", __func__, sensor_idx, pwr_status, ppwr_info->pin);
                if (is_project(23112) || is_project(23113) || is_project(23306)) {
                    if (ppwr_info->pin == IMGSENSOR_HW_PIN_DVDD && (sensor_idx == 0)){
                        aw37004_camera_power_up(OUT_DVDD1, 1110);
                ...
                } else if (is_project(23282) || is_project(23283)) {
                    if (ppwr_info->pin == IMGSENSOR_HW_PIN_DVDD && (sensor_idx == 0)) {
                        aw37004_camera_power_up(OUT_DVDD1, 1224);
