				  ...
				  struct cmdq_pkt *handle)
{
	...
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp;
	struct mtk_ddp_comp *comp_temp;
	int i, j;
	int cur_path_idx;
	char *panel_name = NULL;

	comp_temp = mtk_ddp_comp_request_output(mtk_crtc);
	mtk_ddp_comp_io_cmd(comp_temp, NULL, GET_PANEL_NAME, &panel_name);

//	cfg.w = crtc->mode.hdisplay;
//	cfg.h = crtc->mode.vdisplay;
	if (!strcmp(panel_name, "ac312_p_3_a0025_dsi_vdo")) {
		if (cfg.h == 2372) {
			cfg.h = 2376;
		}
	}
