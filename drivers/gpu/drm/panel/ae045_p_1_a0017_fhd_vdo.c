...
#include "ae045_p_1_a0017_fhd_vdo.h"
...
static void lcm_elvss_dim_dly_setting(void *dsi, dcs_write_gce cb, void *handle)
{
	int i = 0;

	if (!dsi || !handle) {
		DISP_ERR("%s, dsi or handle param error.\n", __func__);
		return;
	}
				 ...
				 unsigned int level)
{
	...
	if (level == 0) {
		lcm_elvss_dim_dly_setting(dsi, cb, handle);
	}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_info("%s: AE045 lcm ctx->prepared %d\n", __func__, ctx->prepared);

	/* vci 3.0v */
	ctx->vci3p0_enable_gpio = devm_gpiod_get(ctx->dev, "vci-enable", GPIOD_OUT_HIGH);
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	DISP_INFO("%s:[LCD] AE045 lcm lcm ctx->prepared=%d\n", __func__, ctx->prepared);
...
static int lcm_esd_gpio_read(struct drm_panel *panel)
{
	...
	pr_err("AE045 debug for %s frist time esd_err_fg = %d\n", __func__, lcm_esd_err_fg_gpio);
	...
	if (!lcm_esd_err_fg_gpio) {
		...
		if(!lcm_esd_err_fg_gpio) {
			pr_err("AE045 debug for %s triger esd to recovery\n", __func__);
			ret = 1;
		}
	} else {
		ret = 0;
	...
	if (1 == ret) {
		...
		pr_err("AE045 debug for %s ESD check failed: %s\n", __func__, payload);
...
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	...
	register_device_proc("lcd", "AE045_P_1_A0017", "P_1");
/* #ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
...
static const struct of_device_id lcm_of_match[] = {
    {
		.compatible = "ae045,p,1,a0017,fhd,vdo",
	},
	{}
};
...
MODULE_AUTHOR("like");
MODULE_DESCRIPTION("lcm ae045_p_1_a0017_fhd_vdo Panel Driver");
