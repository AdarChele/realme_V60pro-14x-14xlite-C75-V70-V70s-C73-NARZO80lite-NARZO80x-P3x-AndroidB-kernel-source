 ...
 ** OPLUS_EDIT
 ** File: ae152_p_3_a0030_hd_dsi_vdo.c
...
#endif
#include "ae152_p_3_a0030_hd_dsi_vdo.h"
...
{
	jdi_dcs_write_seq_static(ctx, 0xFF, 0x5A, 0xA5, 0x06);
	jdi_dcs_write_seq_static(ctx, 0x07, 0xA4);

	jdi_dcs_write_seq_static(ctx, 0xFF, 0x5A, 0xA5, 0x03);
	jdi_dcs_write_seq_static(ctx, 0x83, 0x20);
	jdi_dcs_write_seq_static(ctx, 0x84, 0x01);

	jdi_dcs_write_seq_static(ctx, 0x80, 0x26);
	jdi_dcs_write_seq_static(ctx, 0x81, 0x26);
	jdi_dcs_write_seq_static(ctx, 0x82, 0x26);

	jdi_dcs_write_seq_static(ctx, 0x88, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x89, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x8A, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x8B, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0xA6, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0xAC, 0xFF);

	jdi_dcs_write_seq_static(ctx, 0x8C, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x8D, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x8E, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x8F, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x90, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x91, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x92, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x93, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x94, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0x95, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0xA7, 0xFF);
	jdi_dcs_write_seq_static(ctx, 0xAD, 0xFF);
	...
	jdi_dcs_write_seq_static(ctx, 0xFF, 0x5A, 0xA5, 0x00);
...
#define VAC (1570)
#define HAC (720)
#define VFP_60hz (2220)
#define VFP_90hz (950)
#define VFP_120hz (310)
...
{
        int lut_index[] = {0, 168, 278, 325, 387, 427, 464, 500, 534, 566, 597, 627, 656, 684, 711, 737, 763, 787, 811, 835, 858, 880,
                                                902, 923, 944, 964, 984, 1004, 1023, 1042, 1061, 1079, 1097, 1115, 1133, 1150, 1167, 1184, 1200, 1216, 1230};
        int lut_value1[] = {0, 4, 10, 14, 21, 33, 46, 69, 87, 107, 128, 150, 173, 197, 222, 248, 275, 302, 330, 358, 387, 416, 446,
                                                479, 509, 541, 572, 604, 636, 669, 702, 735, 769, 803, 837, 871, 905, 938, 973, 1008, 1030};
        ...
        if (index_len == value_len) {
                for (i = 0; i < index_len - 1; i++) {
                        index_start = lut_index[i] * MAX_NORMAL_BRIGHTNESS / 1030;
                        index_end = lut_index[i+1] * MAX_NORMAL_BRIGHTNESS / 1030;
                        value1_start = lut_value1[i] * MAX_NORMAL_BRIGHTNESS / 1030;
                        value1_end = lut_value1[i+1] * MAX_NORMAL_BRIGHTNESS / 1030;
        ...
        /*map_exp[3690] = 3090,map_exp[4000] = 3710*/
        for (j = 3690; j <= 4000; j++) {
                map_exp[j] = map_exp[j - 1] + 2;
        }
}
				 ...
				 unsigned int level)
{
	...
	/*max backlight(0x51 reg) 2916, current 42ma*/
...
static int jdi_probe(struct mipi_dsi_device *dsi)
{
        ...
        pr_info("%s+ jdi_probe enter p_3_a0030, vdo,120hz\n", __func__);
	...
	register_device_proc("lcd", "A0030", "P_3");
...
static const struct of_device_id jdi_of_match[] = {
        {
                .compatible = "ae152,p_3,a0030,hd,dsi,vdo",
        },
        {
        }
};
...
static struct mipi_dsi_driver jdi_driver = {
        ...
        .remove = jdi_remove,
        .driver = {
                .name = "ae152_p_3_a0030_hd_dsi_vdo",
                .owner = THIS_MODULE,
...
MODULE_DESCRIPTION("P_3 A0030 VDO 120HZ LCD Panel Driver");
